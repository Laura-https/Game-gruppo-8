...
function create_single_enemy(s, floor, player, posX, posY, minX, maxX) {
  ...
  // Crea la testa SUBITO, non in update
  const headX = posX;
  const headY = posY - ENEMY_H; 
  e.head = PP.shapes.rectangle_add(s, headX, headY, HEAD_W, HEAD_H, "0x0000ff", 0); 
  PP.physics.add(s, e.head, PP.physics.type.STATIC);
  
  // Collisione testa con player per elimminare nemico
  PP.physics.add_overlap_f(s, player, e.head, (s, p, h) => {
    if (PP.game_state.get_variable("DEAD")) return;
    
    PP.physics.set_velocity_y(p, -450); // Rimbalzo
    PP.assets.destroy(e.head);
    PP.assets.destroy(e);
    // Rimuovi nemico dalla lista
    let idx = enemies_list.indexOf(e);
    if (idx !== -1) {
      enemies_list[idx] = null;
    }
...
create_single_enemy(s, floor, player, 6459, 2090, 6500, 6830); 
...
function update_enemy(s) {
  enemies_list.forEach((e, index) => {
    ...
    if (e.head && e.head.ph_obj && e.head.ph_obj.body) {
