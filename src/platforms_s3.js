...
function collision_platform(s, player, platform) {
   // Tolleranza verticale: usa la costante se disponibile, altrimenti default 10px
   const tol = (typeof PLATFORM_TOLERANCE_Y !== 'undefined') ? PLATFORM_TOLERANCE_Y : 10;

   // Verifico innanzitutto l'overlap orizzontale
   if (player.geometry.x >= platform.geometry.x &&
       player.geometry.x <= platform.geometry.x + platform.geometry.display_width) {
       // Verifico che il player sia vicino al top della piattaforma (cioè sopra di essa)
       if (Math.abs(player.geometry.y - platform.geometry.y) < tol) {
            player.is_on_platform = true;
           jumpCount = -1; //------mettere -1 se no inspiegabilmente non salta due volte
       }
   }
...
function create_platforms_s3(s, player) {
    ...
    PP.physics.add_collider_f(s, player, moving_plat, collision_platform);
